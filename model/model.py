...
class BipartiteGraphConvolution(torch_geometric.nn.MessagePassing):
    ...
    def __init__(self):
        ...
        self.feature_module_final = torch.nn.Sequential(
            PreNormLayer(1, shift=False), torch.nn.ReLU(),
            torch.nn.Linear(emb_size, emb_size))
    ...
    def forward(self, left_features, edge_indices, edge_features,
                right_features):
        output = self.propagate(edge_indices,
                                size=(left_features.shape[0],
...
class GNNPolicy(BaseModel):
    ...
    def forward(self, constraint_features, edge_indices, edge_features,
        ...
        constraint_features = self.conv_v_to_c(variable_features,
